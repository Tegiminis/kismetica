...
class Job:
    ...
    def add_traits(self, target, level):
        ...
        if level in self.traits:
            target.perks.add(self.traits[level])
...
def check_for_level(target, job):
    ...
    if utils.inherits_from(job, str):
        sc_id = job
    if utils.inherits_from(job, Job):
        sc_id = job.id

    if sc_id not in subclasses.keys():
        return
